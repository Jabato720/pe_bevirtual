...
// Import Bracket Dashboard components
import BracketSidebar from '../components/navigation/BracketSidebar';
import BracketHeader from '../components/navigation/BracketHeader';
import { RevenueMetricCard, ClientsMetricCard, EbitdaMetricCard, CostsMetricCard } from '../components/dashboard/BracketMetricCard';

// Import Bracket Dashboard styles
...
const ProfessionalCalculatorBracket = () => {
  const { id } = useParams();
      ...
      <div className="br-layout">
        <div className="d-flex align-items-center justify-content-center" style={{ minHeight: '100vh' }}>
          <div className="text-center">
            <div className="spinner-border spinner-border-lg tx-primary" role="status">
              <span className="visually-hidden">Loading...</span>
            </div>
            <div className="tx-muted mg-t-10">Cargando plan financiero...</div>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="br-layout">
      ...
      {/* Bracket Header */}
      <BracketHeader
        title={planName || 'Plan Financiero'}
        subtitle={selectedBusinessModel ? `${selectedBusinessModel.nombre} - ${selectedBusinessModel.sector}` : 'Dashboard Profesional'}
      ...
      <div className="br-mainpanel">
        {error && (
          <div className="br-card" style={{ marginBottom: '1rem' }}>
            <div className="br-card-body bg-light-danger">
              <div className="tx-danger tx-medium">{error}</div>
            </div>
          </div>
        )}

        {/* Metrics Overview - Only show if professional results available */}
        {professionalResults && (
          <div className="row row-sm mg-b-20">
            <div className="col-xl-3 col-lg-6">
              <RevenueMetricCard
                value={professionalResults.financialMetrics?.ingresosAno1 || 0}
                subtitle="Ingresos primer año"
                trend="up"
                trendValue="+12.5%"
                loading={!professionalResults.financialMetrics}
              />
            </div>
            <div className="col-xl-3 col-lg-6">
              <ClientsMetricCard
                value={professionalResults.customerMetrics?.clientesFinAno1 || 0}
                subtitle="Clientes fin año 1"
                trend="up"
                trendValue="+8.3%"
                loading={!professionalResults.customerMetrics}
              />
            </div>
            <div className="col-xl-3 col-lg-6">
              <EbitdaMetricCard
                value={professionalResults.financialMetrics?.ebitdaAno1 || 0}
                subtitle={`Margen: ${professionalResults.financialMetrics?.margenEbitdaAno1?.toFixed(1) || 0}%`}
                trend={professionalResults.financialMetrics?.ebitdaAno1 >= 0 ? "up" : "down"}
                trendValue={professionalResults.financialMetrics?.ebitdaAno1 >= 0 ? "Positivo" : "Negativo"}
                loading={!professionalResults.financialMetrics}
              />
            </div>
            <div className="col-xl-3 col-lg-6">
              <CostsMetricCard
                value={calculatedResults.totalCostesFijos * 12}
                subtitle="Costes fijos anuales"
                trend="down"
                trendValue="Optimizado"
                loading={loading}
              />
            </div>
          </div>
        ...
        {/* Footer Actions - Now integrated into main panel */}
...
export default ProfessionalCalculatorBracket;