...
const Register = () => {
  ...
  const { name, email, password, password2, company_name } = formData;
  ...
  const onSubmit = async e => {
    ...
    if (!success) {
      setError('Error al registrar el usuario. Puede que el email ya esté en uso.');
    }
      ...
      <div className="max-w-md w-full space-y-8">
        <div>
          ...
          <p className="mt-2 text-center text-sm text-gray-600">
            ...
            <Link to="/login" className="font-medium text-accent-color hover:text-blue-500">
              Inicia sesión
            </Link>
          </p>
        </div>
        ...
        <form className="mt-8 space-y-6" onSubmit={onSubmit}>
          <div className="rounded-md shadow-sm -space-y-px">
            ...
            <div>
              <label htmlFor="company_name" className="sr-only">Nombre de la Empresa</label>
              <input
                id="company_name"
                name="company_name"
                type="text"
